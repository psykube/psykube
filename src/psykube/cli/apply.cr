...
class Psykube::Commands::Apply < Admiral::Command
  ...
  {% if !system("which docker || true").empty? %}define_flag push : Bool, description: "Build and push the docker image."{% end %}
  ...
  def run
    ...
    {% if !system("which docker || true").empty? %}docker_build_and_push(generator.image) if !image && flags.push{% end %}
